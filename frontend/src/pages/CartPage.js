...
const CartPage = () => {
  ...
  const handleGuestCheckout = async () => {
    ...
    try {
      ...
      const result = await apiService.guestCheckout({
        email: guestEmail,
        items: checkoutItems
      });
  ...
  const handleGoogleSignInForUser = async () => {
    try {
      setShowGuestModal(false);
      ...
      if (result.success) {
        ...
        // Store that this is for checkout flow (not regular sign-in)
        sessionStorage.setItem('google_oauth_checkout_flow', 'true');
        ...
        setShowGuestModal(true); // Reopen modal on error
      }
    } catch (error) {
      ...
      <Modal isOpen={showChoiceModal && !isAuthenticated} onClose={handleCloseModals} preventClose={false}>
        <ModalContent>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: theme.spacing[3] }}>
            ...
            >
              <svg width="18" height="18" viewBox="0 0 24 24" style={{ marginRight: theme.spacing[1] }}>
        ...
        <ModalContent>
          ...
          <ButtonGroup>
            ...
            <Button 
              ...
              disabled={isProcessing}
            >
              {isProcessing ? 'Processing...' : 'Continue as Guest (1 Download)'}
            </Button>
          </ButtonGroup>
          ...
          <Button 
            ...
            size="lg" 
            fullWidth
            onClick={handleGoogleSignInForUser}
            disabled={isProcessing}
            style={{
              background: '#4285f4',
              color: 'white',
          ...
          >
            ...
            Sign In with Google & Get 3 Downloads
          </Button>
          ...
          <p style={{
            ...
            💡 <strong>Tip:</strong> Sign in with Google to get 3 downloads per purchase instead of 1!
          </p>
        </ModalContent>
      </Modal>
